...
def one_hot_encode(data):
    for col in data.columns[100:]:
        if col == 'Label':
            continue
        data = pd.concat([data, pd.get_dummies(data[col], prefix=col).astype(int)], axis=1)
...
def k_NN(k, train_data, test_data, normalisation_method, distance_metric):
    ...
    for i in range(len(test_data)):
        ...
        for j in range(len(train_data)):
            distances.append((distance_metric(test_data.iloc[i], train_data.iloc[j]),j))
        sorted_distances = sorted(distances, key=lambda x: x[0])
        
        k_nearest = sorted_distances[:k]
        k_nearest_labels = [train_data.iloc[x[1]]['Label'] for x in k_nearest]
        
        result[test_data.iloc[i].name] = max(set(k_nearest_labels), key=k_nearest_labels.count)
